...
NUM_CARDS   = matrix.shape[0]   # 56
NUM_SYMBOLS = matrix.shape[1]   # 56
...
canvas.sel { border:3px solid var(--warn); }
...
/* Flash animacio encert */
@keyframes flashOk { 0%{opacity:1} 50%{opacity:0.3} 100%{opacity:1} }
@keyframes flashErr { 0%{transform:translateX(0)} 25%{transform:translateX(-6px)}
  75%{transform:translateX(6px)} 100%{transform:translateX(0)} }
.flash-ok { animation: flashOk 0.3s ease; }
.flash-err { animation: flashErr 0.3s ease; }
/* Bonus temps */
...
function updateStreak() {
  ...
  var sk = document.getElementById('stk');
  if (G.streak >= 2) {
    el.style.display = 'block';
    sk.textContent = G.streak;
  } else {
...
function nova() {
  ...
  c1.classList.remove('sel'); c2.classList.remove('sel');
...
function clic(cv, esq, e) {
  ...
  var hitR = Math.floor(cv.width / 2 / 5.5) + 4;
  var cl = null;
  sy.forEach(function(s, i) {
    if (Math.hypot(mx - ps[i][0], my - ps[i][1]) < hitR) cl = s;
  });
  if (cl === null) return;
  var c = comuns(G.i1, G.i2);
  if (esq) {
    ...
    setRes(G.estat === 'un' ? 'ok1' : '', 'Seleccionat: #' + cl,
