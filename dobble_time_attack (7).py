...
# ── Tot l'HTML/JS com a string normal, sense f-string ──
# Les dades s'injecten amb .replace() al final
page = '''<!DOCTYPE html>
<html lang="ca">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
<title>Dobble Contrarellotge</title>
<style>
:root {
...
function draw(cv, idx, hl, dm) {
  ...
  function rnd() {
    ...
    sy.forEach(function(s, i) {
      var x = ps[i][0], y = ps[i][1], r = Math.floor(R / 4.8);
...
function clic(cv, esq, e) {
  ...
  var cl = null;
  sy.forEach(function(s, i) {
    if (Math.hypot(mx - ps[i][0], my - ps[i][1]) < 36) cl = s;
  });
  if (cl === null) return;
  var c = comuns(G.i1, G.i2);
  if (esq) {
...
// Botons: gestio touch + click sense doble-fire
function btn(id, fn) {
...
# Injectem les dades amb replace (cap f-string al JS → cap risc de corrompre {})
